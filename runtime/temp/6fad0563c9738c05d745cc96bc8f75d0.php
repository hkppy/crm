<?php /*a:3:{s:78:"C:\phpStudy2018\PHPTutorial\WWW\tp5.1\application\demo\view\customer\edit.html";i:1539763666;s:78:"C:\phpStudy2018\PHPTutorial\WWW\tp5.1\application\demo\view\public\header.html";i:1539669711;s:83:"C:\phpStudy2018\PHPTutorial\WWW\tp5.1\application\demo\view\public\header_menu.html";i:1539738244;}*/ ?>
<!DOCTYPE html>
<html>
	...
	<body>
		...
		<form action="" method="post" >
			...
			<div class="agent">
				<div class="agent-title">修改客户</div>
				...
				<div class="ear-list list-title"><span class="span1">联系号码：</span>
					<span> <input type="text"  class="span3" name="lxfs_value" id="lxfs_value" ></span>
				</div>
				

				<div class="ear-list list-title"><span class="span1" >密码：</span>
					<span> <input type="text"  class="span3" placeholder="选填" name="password" id="password"  ></span>
				</div>
				
				<div class="ear-list list-title"><span class="span1">姓名：</span>
					<span> <input type="text"  class="span3" name="realname"  placeholder="选填" id="realname" ></span>
				</div>
				...
				<div class="ear-list list-title"><span class="span1">出生日期：</span>
					<span> <input type="text"  class="span3" name="birthday" id="birthday" ></span>
				</div>
				
				
				<div class="ear-list list-title"><span class="span1">详细地址：</span>
					<span> <input type="text"   class="span3" name="address"  placeholder="选填" id="address" ></span>
				</div>
				<div class="ear-list list-title"><span class="span1">客户备注：</span>
					<span><input type="text"  class="span3" name="notes"  placeholder="选填" id="notes" ></span></div>
 ...
 <script type="text/javascript">
	$(function() {
		...
		$("#btn").click(function() {
			...
			$.post({
				url: "<?php echo url('api/customer/edit_post'); ?>",
				data: $("form").serialize(),
...
<script>
	...
	$(function(){
		$.post(
			"<?php echo url('api/customer/edit'); ?>",
			{id:'<?php echo htmlentities($id); ?>',b:'value2'},
			function(data){
				if(data.status==1){
					console.log(data);
					var list=data.data;
					var customer_info=list.customer_info;
					console.log(customer_info.lifa);
					$("#lxfs_value").val(list.lxfs_value);
					$("#id").val(list.id);
					$("#password").val(list.password);
					$("#address").val(customer_info.address);
					$("#realname").val(customer_info.realname);
					$("#customer_info_id").val(customer_info.id);
					$("#notes").val(customer_info.notes);
					$("#birthday").val(customer_info.birthday);
					$(".lxfs input:radio[value="+list.lxfs2+"]").attr('checked','true');
					
					$(".lifa input:radio[value="+customer_info.lifa+"]").attr('checked','true');
				}else{
					layer.msg("信息获取失败");
				}
		},'json');
